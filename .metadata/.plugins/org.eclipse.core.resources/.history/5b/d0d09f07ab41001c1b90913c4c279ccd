...
public class PesoService implements PesoInterface{
	...
	@Override
	public List<PesoDAO> getPeso(Long codigoCliente) {
	...
	public void setPeso(PesoDAO dao) {
		...
		try {
			...
			if(peso.getCodigo()==null) {
				...
				stmt.setLong(1, peso.getCodigoUsuario());
					
				Date data = new Date(peso.getData().getTimeInMillis());

				stmt.setDate(2, data);
				
				stmt.setDouble(3, peso.getValor());
			}else {
	...
	@Override
	public void deletePeso(Long codigoCliente, Long codigoPeso) {
		PreparedStatement stmt= null;
